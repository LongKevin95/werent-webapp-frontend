...
import { ChevronDown, ChevronLeft, ChevronRight, ChevronUp, FileCheck2, LoaderCircle, ShieldCheck, UserRoundCheck, X } from "lucide-react";
...
const statusBadgeClass=(status)=>status==="verified"?"bg-green-50 text-green-700":status==="rejected"?"bg-red-50 text-red-700":status==="need_more_info"?"bg-blue-50 text-blue-700":"bg-amber-50 text-amber-700";
...
function isImageDocument(document) {
  return document.mimeType?.startsWith("image/") ||
    /\.(avif|gif|jpe?g|png|webp)$/i.test(`${document.url ?? ""} ${document.originalName ?? ""}`);
}
...
function DocumentGallery({ accessToken, documents, onError }) {
  ...
  const imageDocuments = useMemo(() => safeDocuments
    .map((document, index) => ({ document, index }))
    .filter(({ document }) => isImageDocument(document)), [safeDocuments]);
  const otherDocuments = useMemo(() => safeDocuments
    .map((document, index) => ({ document, index }))
    .filter(({ document }) => !isImageDocument(document)), [safeDocuments]);
  const activeImage = activeIndex === null || activeIndex >= imageDocuments.length ? null : imageDocuments[activeIndex];
  ...
  useEffect(() => {
    ...
    async function loadImages() {
      const entries = await Promise.all(imageDocuments.map(async ({ document, index }) => {
        if (!document.url) return null;
        if (!document.url.startsWith("/api/uploads/werent/kyc")) return [index, document.url];
        const response = await fetch(`${getApiBaseUrl()}${document.url}`, {
          headers: { Authorization: `Bearer ${accessToken}` },
        });
        ...
        <div className="mt-3 grid gap-2">
          {otherDocuments.map(({ document, index }) => (
            <button
              ...
              key={`${document.publicId || document.url}-${index}`}
              type="button"
              onClick={() => openVerificationDocument(document, accessToken, onError)}
            >
...
function Detail({ accessToken, item, kind, onError, onReview, busy }) {
  ...
  const fields=kind==="accounts"?[["Họ tên",item.fullName],["Ngày sinh",fmt(item.dateOfBirth)],["Email",item.email],["Số điện thoại",item.phone],["Địa chỉ",item.address],["Số CCCD",item.identityNumber],["Ngày cấp",fmt(item.identityIssuedAt)],["Số hộ Chiếu",item.passportNumber],["Mã số thuế",item.taxCode]]:[["Loại BĐS",property?.propertyType],["Dự án",property?.projectName||"—"],["Địa chỉ",property?.address],["Diện tích",property?.area?`${property.area} m²`:"—"],["Phòng ngủ",property?.bedrooms??"—"],["Phòng tắm",property?.bathrooms??"—"],["Trạng thái tin",property?.status]];
  ...
  <div className="mt-4"><h4 className="font-semibold">Giấy tờ đã tải lên</h4><DocumentGallery accessToken={accessToken} documents={item.documents} onError={onError}/></div>
...
export default function AdminKycPage({ accessToken, onNotify = () => {} }) {
  ...
  useEffect(()=>{if(error)onNotify(error,"error")},[error,onNotify]);
  ...
  async function review(payload){setBusy(true);setError("");try{await reviewAdminKycRequest(accessToken,kind,selected._id,payload);onNotify("Cập nhật hồ sơ xác thực thành công.");setRefresh((v)=>v+1)}catch(e){setError(e.message)}finally{setBusy(false)}}
  ...
  <div className="mt-4 flex flex-wrap gap-2">{Object.entries(statuses).map(([key,label])=><button className={`rounded-lg px-3 py-2 text-sm ${status===key?"bg-[#159848] text-white":"border bg-white"}`} key={key} onClick={()=>setStatus(key)}>{label}</button>)}</div>
  <section className="mt-4 grid overflow-hidden rounded-2xl border bg-white xl:grid-cols-[minmax(0,1fr)_390px]"><div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="p-4">Mã hồ sơ</th><th>Người gửi</th><th>{kind==="accounts"?"CCCD":"Tin đăng"}</th><th>Ngày gửi</th><th>Trạng thái</th></tr></thead><tbody>{loading?<tr><td className="p-12 text-center" colSpan="5"><LoaderCircle className="mx-auto animate-spin text-green-600"/></td></tr>:items.length?items.map((item)=><tr className={`cursor-pointer border-t hover:bg-green-50/50 ${selected?._id===item._id?"bg-green-50":""}`} key={item._id} onClick={()=>open(item)}><td className="p-4 font-semibold">#{item._id.slice(-8).toUpperCase()}</td><td>{item.user?.fullName||item.fullName}<p className="text-xs text-slate-400">{item.user?.email||item.email}</p></td><td>{kind==="accounts"?item.identityNumber:item.property?.title}</td><td>{fmt(item.createdAt)}</td><td><span className={`rounded-full px-2 py-1 text-xs ${statusBadgeClass(item.status)}`}>{statuses[item.status]}</span></td></tr>):<tr><td className="p-12 text-center text-slate-500" colSpan="5">Không có hồ sơ phù hợp.</td></tr>}</tbody></table></div><Detail accessToken={accessToken} busy={busy} item={selected} kind={kind} onError={setError} onReview={review}/></section></div>;
}
